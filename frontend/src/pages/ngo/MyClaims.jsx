...
const MyClaims = () => {
    ...
    const handlePickupMyself = async (donationId) => {
        setIsProcessing(true);
        try {
            await api.updateDonation(donationId, { status: 'PICKED_UP' });
    ...
    const openAssignModal = (donationId) => {
        setSelectedDonationId(donationId);
            ...
            {showDistributionModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-fade-in">
                    <div className="bg-white rounded-[3rem] shadow-2xl max-w-md w-full p-10 transform transition-all border border-slate-100">
                        ...
                        {error && <p className="text-red-500 text-[10px] font-black uppercase tracking-widest text-center mb-6">{error}</p>}
