...
const DonationDetails: React.FC = () => {
    ...
    const isDonor = user?.id === donation?.donorId || user?.id === (donation?.donorId as any)?._id || user?.id === (donation?.donorId as any)?.id;
    ...
    const handlePickupMyself = async () => {
        ...
        setIsProcessing(true);
        try {
            await api.updateDonation(donation.id, { status: 'PICKED_UP' as any });
            const updated = await api.getDonationById(donation.id);
            setDonation(updated);
    ...
    const handleOpenEditModal = () => {
        ...
        setEditFormData({
            ...
            latitude: (donation as any).location?.coordinates[1] || null,
            longitude: (donation as any).location?.coordinates[0] || null
        });
                        ...
                        <div className="bg-white p-10 rounded-[3rem] shadow-xl shadow-slate-200/50 border border-slate-100 animate-slide-up" style={{ animationDelay: '0.45s' }}>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                ...
                                <div className="p-8 bg-slate-50 rounded-[2.5rem] border border-slate-100">
                                    ...
                                    <div className="space-y-4">
                                        {(donation as any).donorId?.phone && (
                                            <div className="flex items-center text-sm font-bold text-slate-600">
                                                <FiPhone className="mr-3 text-primary-500" />
                                                {(donation as any).donorId?.phone}
                                            </div>
                                        )}
                                        {(donation as any).donorId?.email && (
                                            <div className="flex items-center text-sm font-bold text-slate-600">
                                            ...
                                            <button
                                                ...
                                                onClick={() => {
                                                    ...
                                                    if (removedSrc.startsWith('blob:')) {
                                                        // It was a new file, find which one in proofFiles
                                                        // We can't easily match indices if they aren't 1:1
                                                        // Let's just filter proofFiles by keeping only those whose blob URLs still exist in the NEW previewImages
                                                        // But we haven't updated previewImages yet in this scope...
                                                        // Let's just track them better.
                                                        setProofFiles(prev => {
                                                            const newFiles = [...prev];
                                                            // We need to know which file this blob belonged to.
                                                            // When adding we correspond them.
                                                            // For now, just remove by finding index relative to other blobs
