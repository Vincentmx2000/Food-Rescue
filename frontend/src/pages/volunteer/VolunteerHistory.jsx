...
const VolunteerHistory = () => {
    const { user } = useAuth();
    const [deliveries, setDeliveries] = useState([]); // Using any[] to handle the Task + nested Donation structure
    ...
    const handleMarkPickedUp = async (taskId) => {
    ...
    const handleMarkDistributed = async (taskId) => {
        setCompletingId(taskId);
        try {
            const delivery = deliveries.find(d => d.id === taskId);
            const donationId = typeof delivery.donation === 'string'
                ...
                {deliveries.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                        {deliveries.map((delivery) => {
                            const isMyTask = delivery.volunteerId === user?.id || delivery.volunteerId === user?._id;
